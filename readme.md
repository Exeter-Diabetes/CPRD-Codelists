...
*Exception: For most biomarkers (24hr urine protein, ACR, blood albumin, ALT, AST, bilirubin, blood creatinine, urine creatinine, FE1, fasting glucose, haematocrit, haemoglobin, HDL, LDL, PCR, total cholesterol, tryglycerides, TSH, TTG, urine dipstick protein) we have used the NHS Pathology Bounded Code List (PBCL), a subset of codes used for the majority of test reporting. We term searched the PBCL list for each biomarker (PBCL codes identified are available [here](https://github.com/Exeter-Diabetes/CPRD-Codelists/tree/main/Medcodes/Biomarkers/PBCL%20codelists)) and then mapped the resulting Read/SNOMED codes to medcodes (clinician review not necessary). In addition, a subset of medcode lists were generated by alternative means: Covid medcodes were provided by CPRD; ethnicity medcodes were provided by LSHTM; family history, frailty ('frailty_simple'), diabetes, and various biomarker medcodes (urine albumin, C-peptide, cholesterol:HDL ratio, GAD/IA2/ICA antibodies, HbA1c) were generated by searching for key words/terms in the CPRD Aurum Medical Dictionary for completeness. Read and SNOMED codelists are not available for these conditions/biomarkers.
